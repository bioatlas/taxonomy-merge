################################################################################################
# prep-asv-for-tax-merge.R     
# Author: Maria Prager

# Reads ASV annotation file (See data/asv-for-r-prep.tsv for structure)
# For each row, moves from low to high rank and adds id, name, missing ancestors (as new rows)
# Calculates taxonID as md5 sum
# Parses rows again, and looks up id of closest parent to each row
# Outputs tsv-file to use in taxonomy-merge
# To Do: edit add.ancestors to not use positional matching of cols, replace loops w apply, if faster
...
# Add id, name, rank - for ASVs only
add.asv.specs <- function(df) {
  for(j in 1:nrow(df)) {
    row <- df[j,]
    taxonid <- row$asv
...
add.ancestors <- function(df, ranks) {
  for(j in 1:nrow(df)) {
    ...
    for (i in rev(ranks)) { 
      # If rank is empty or ASV, go to next
      if (is.na(row[[i]])) {next}
      name.rank <- get.name.n.rank(row, i)
      taxonid <- sum.id('SBDI', row[ranks])
...
annot.file <- 'data/asv-for-r-prep.tsv'
# Filter on taxa
taxa <- c('Edaphobacter')
...
# Filter on taxon names
filt.df <- concat.filtered(df, taxa)

# Save to tsvs
write.table(df, file = "~/data/lucene/sources/asv-full.tsv", sep='\t', row.names = F, na = '', quote = FALSE)
write.table(filt.df, file = "~/data/lucene/sources/asv-filt.tsv", sep='\t', row.names = F, na = '', quote = FALSE)
